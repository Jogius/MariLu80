...
export default function UploadText() {
    ...
    async function handleSubmit(e) {
        ...
        await Axios.post('/php/uploadText.php', formData, {'content-type': 'multipart/form-data'})
            .then(r => {
                resetForm();
                    ...
                    <div>
                        ...
                        <fieldset id="text">
                            ...
                            <label id="filedrag" htmlFor="file" onDragOver={fileDragHover} onDragLeave={fileDragHover}
                                   onDrop={handleFileChange}>{t('uploadtext.form.text.placeholder.filedrag')}</label>
                            <div className="filename">{file.name || t('uploadtext.form.text.nofilename')}</div>
                            <textarea id="textfield" name="textfield"
                                      placeholder={t('uploadtext.form.text.placeholder.text')}
                                      maxLength="65534" value={text} onChange={e => setText(e.target.value)}/>
                        </fieldset>
